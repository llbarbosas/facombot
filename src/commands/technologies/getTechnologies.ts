...
export const getTechnologies: Command = {
  ...
  async execute(_, message) {
    const technologyRoles = filterTechnologyRoles(
      await message.getGuildRoles()
    );
    const technologyRoleNames = technologyRoles.reduce(
      (text, role) => `${text}\t\`${role.name}\``,
      ""
    );

    await message.sendChannelMessage(
      "Tecnologias disponíveis disponíveis:" + technologyRoleNames
    );
